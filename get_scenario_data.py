...
scenarios = [
    ...
    '--exclude-module no_new_thermal_capacity switch_model.hawaii.heco_outlook_2020_06 '
    '--include-module switch_model.hawaii.heco_plan_2020_06 '
...
**** main inputs, even though it is set for 2020-22 in inputs_annual
...
for inputs_dir in ['inputs', 'inputs_annual']:
    ...
    df.loc[(df['gen_tech']=='Kahe_5') | (df['gen_tech']=='Kahe_6'), 'gen_max_age'] \
        += (2029 - 2045)
    # also correct retirement dates for all Kahe and Waiau units
    units = ['Kahe_{}'.format(u) for u in range(1, 6+1)] + ['Waiau_{}'.format(u) for u in range(3, 8+1)]
    for u in units:
        df.loc[df['gen_tech']==u, 'gen_max_age'] -= 1
